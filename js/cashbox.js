...
(function () {
  ...
  async function render(page) {
    const [income, expenses] = await Promise.all([
      window.store.list('income'), window.store.list('expenses'),
    ]);
    const methodOpts = METHODS.map(m => '<option>' + m + '</option>').join('');

    // שכר לימוד הוסר מהמערכת (2026-08-23) — גוף חיצוני מטפל בגבייה.
    // הקופה מציגה כעת הכנסות והוצאות בלבד.
    ...
    page.innerHTML =
      ...
      '<div class="stat-row">' +
        '<div class="stat-card"><div class="stat-ic"><i class="bi bi-plus-circle"></i></div><div class="stat-num">' + ILS(incomeSum) + '</div><div class="stat-lbl">הכנסות</div></div>' +
